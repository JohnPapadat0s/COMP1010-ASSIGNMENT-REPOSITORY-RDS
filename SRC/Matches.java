...
public class Matches {
    ...
    public static void simulatingMatches(ArrayList<Team> allTeams) {
        ...
        while (allTeams.size() > 1) {
            ...
            // if theres an odd number of teams in the arraylist, the last team will
            // automatically pass to the next round.
            if (allTeams.size() % 2 != 0) {
                allTeams.get(allTeams.size() - 1).hasWon = true;
                System.out.println(allTeams.get(allTeams.size() - 1).TeamName
                        + " procedes to the next round due to an odd number of teams!");
                System.out.println("");
            }
             ...
             * e.g. Previously if 5 teams are in the tournament, the last team in the array
             * skips strait to the finals.
